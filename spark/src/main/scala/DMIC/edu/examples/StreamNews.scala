...
object StreamNews {
  ...
  var LRmodel5 = new LogisticRegressionModel(Vectors.zeros(50), 0.0).clearThreshold()
  var LRmodel6 = new LogisticRegressionModel(Vectors.zeros(50), 0.0).clearThreshold()
  var LRmodel7 = new LogisticRegressionModel(Vectors.zeros(50), 0.0).clearThreshold()
  ...
  def main(args: Array[String]): Unit = {
    ...
    train.foreachRDD(rdd =>
      if(!rdd.isEmpty()) {
        ...
        LRmodel5 = svm.run(rdd.map(x => x match {
          case LabeledPoint(5,_) => LabeledPoint(1,x.features)
          case _ => LabeledPoint(0,x.features)}),LRmodel4.weights).clearThreshold()

        LRmodel6 = svm.run(rdd.map(x => x match {
          case LabeledPoint(6,_) => LabeledPoint(1,x.features)
          case _ => LabeledPoint(0,x.features)}),LRmodel4.weights).clearThreshold()

        LRmodel7 = svm.run(rdd.map(x => x match {
        ...
        val temp = LRmodel1.predict(rdd.map(_.features))
          .zip(LRmodel2.predict(rdd.map(_.features)))
          .zip(LRmodel3.predict(rdd.map(_.features))).map(x => List(x._1._1,x._1._2,x._2))
          .zip(LRmodel4.predict(rdd.map(_.features))).map(x => x._1 :+ x._2)
          .zip(LRmodel5.predict(rdd.map(_.features))).map(x => x._1 :+ (x._2))
          .zip(LRmodel6.predict(rdd.map(_.features))).map(x => x._1 :+ (x._2))
          .zip(LRmodel7.predict(rdd.map(_.features))).map(x => x._1 :+ (x._2)).map(x => x.zipWithIndex.maxBy(_._1)._2+1.0)
