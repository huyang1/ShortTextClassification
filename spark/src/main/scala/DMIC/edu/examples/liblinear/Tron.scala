...
class Tron(val function : TronFunction)
{
	 ...
	 */
	def tron(prob : Problem, param : Parameter, eps : Double, weights : org.apache.spark.mllib.linalg.Vector) : DenseVector[Double] =
	{
		val ITERATIONS = 100
		...
		var (search, iter) = (1, 1)
		var w = DenseVector(weights.toArray)
